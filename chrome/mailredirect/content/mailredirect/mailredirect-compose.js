...
function FillIdentityList(menulist)
{
  ...
  for (let acc = 0; acc < accounts.length; acc++) {
    ...
    for (let i = 0; i < identities.length; i++) {
      ...
      let item = menulist.appendItem(server.prettyName, identity.key,
                                     uid);
...
*/
  ...
  // mantis 0004853: Ajout d'une fonctionnalité de signalement de spam
  if (params && 2<window.arguments.length){
    var inputField = awGetInputElement(1);
    inputField.value=window.arguments[2];
  }
  ...
  if (params && 2<window.arguments.length){
    // mantis 0004853: Ajout d'une fonctionnalité de signalement de spam
    let msgCompFields=params.composeFields;
...
//      var appInfo = Cc["@mozilla.org/xre/app-info;1"].getService(Ci.nsIXULAppInfo);
  ...
  _stopMeteors: function() {
    ...
    // mantis 0004853: Ajout d'une fonctionnalité de signalement de spam
    if (3<window.arguments.length &&
        window.arguments[3]){
      if (!success)
        DoCommandClose();
      window.arguments[3](success, mstate.size);
    }
  }
};
